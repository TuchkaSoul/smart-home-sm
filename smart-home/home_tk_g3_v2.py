...
class SmartHomeApp:
    ...
    def get_flow_between_cells(self, y1, x1, y2, x2):
        """
        Улучшенный расчет теплообмена с:
        - Нелинейной зависимостью
        - Ограничением температуры
        - Усиленным влиянием улицы
        - Стабильными параметрами
        """
        # Ограничиваем температуру в пределах [-100, 200]
        self.temp_matrix = np.clip(self.temp_matrix, -100, 200)
        
        cell1_type = self.type_matrix[y1, x1]
        cell2_type = self.type_matrix[y2, x2]
        temp1 = self.temp_matrix[y1, x1]
        temp2 = self.temp_matrix[y2, x2]
        
        # Усиленное влияние уличной температуры
        if cell1_type == OUTSIDE or cell2_type == OUTSIDE:
            outside_temp = self.initial_data[TEMPERATURES][OUTSIDE_TEMP]
            if cell1_type == OUTSIDE:
                return (outside_temp - temp2) * 0.3  # Коэффициент усиления уличного влияния
            else:
                return (outside_temp - temp1) * 0.3
        ...
        if cell1_type in {DOOR, WINDOW} or cell2_type in {DOOR, WINDOW}:
            ...
            if cell1_type == DOOR or cell2_type == DOOR:
                base_flow = self.initial_data[COEFFICIENTS][OPEN_DOOR_FLOW]
            else:
                base_flow = self.initial_data[COEFFICIENTS][WINDOW_FLOW]
            
            # Нелинейная зависимость с насыщением
            return base_flow * math.tanh(temp_diff / 10) * 5  # tanh дает плавное насыщение

        # Теплообмен через стену
        if cell1_type == WALL or cell2_type == WALL:
            temp_diff = temp2 - temp1
            wall_resist = self.initial_data[COEFFICIENTS][WALL_RESISTANCE]
            return temp_diff * wall_resist * math.exp(-abs(temp_diff)/50)  # Экспоненциальное ослабление

        # Обычный воздушный теплообмен
        temp_diff = temp2 - temp1
        air_flow = self.initial_data[COEFFICIENTS][AIR_FLOW]
        
        # Комбинированная нелинейная зависимость
        flow = air_flow * temp_diff / (1 + abs(temp_diff)/20)
        return min(max(flow, -5), 5)  # Жесткое ограничение потока

    def calculate_temp_delta(self, y, x, current_temp):
        """Улучшенный расчет изменения температуры"""
        total_delta = 0
        neighbor_count = 0
        outside_influence = 0  # Дополнительное влияние улицы

        neighbors = [(y-1, x), (y+1, x), (y, x-1), (y, x+1)]
        for ny, nx in neighbors:
            if 0 <= ny < GRID_ROWS and 0 <= nx < GRID_COLS:
                cell_type = self.type_matrix[ny, nx]
                
                # Усиленное влияние уличных клеток
                if cell_type == OUTSIDE:
                    outside_influence += (self.temp_matrix[ny, nx] - current_temp) * 0.5
                    continue
        ...
        # Усреднение с учетом уличного влияния
        avg_delta = total_delta / max(neighbor_count, 1)
        return avg_delta + outside_influence / 4  # Комбинированный эффект

    def update_temperature(self):
        """Обновление температур с защитой от переполнения"""
